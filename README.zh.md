...
- **直接点名、特定宿主(`/codex-fix`)。** 专门为一个工作流服务的薄壳:在当前分支上运行 `/codex:rescue`,把 findings 交给 `refactor-verify` 的 review-driven fix mode,配合 4-check 验证一起解决。**只在 Claude Code 加 Codex 插件** 安装时动,其他任何宿主下都只输出一行 fallback 然后干净地退出。可移植引擎在 `/refactor-verify` 里 —— 如果你的 review 来源不是 Codex(人工 PR review、Sentry、扫描器、手动粘贴的笔记),直接调 `/refactor-verify` 就行。
- **直接点名、编排器(`/ship-cycle`)。** pack 里第一个 process 分类的 skill。输入可以是粘贴的清单、`/vibesubin` sweep 报告,或一个具体范围(*"perf in src/api/"*),产出一套规范的多语言 issue,聚合成和下一个 semver 版本 1:1 对应的 milestone,按 label 把每个 issue 分派给对应 worker 带验证执行,milestone 关闭时从已关闭 issue 聚合出功能型 changelog,打 annotated tag 并创建 GitHub release。direct-call only,不参与 `/vibesubin` 并行 sweep。前提是 GitHub 仓库加已认证的 `gh` CLI;其他任何情况(非 GitHub remote、`gh` 没登录、宿主里压根没装 `gh`)都只输出一行 fallback 然后干净地退出。

有三个 skill 不管你怎么调都不会编辑文件(纯诊断):**`fight-repo-rot`**(找死代码和气味,删除交给 `refactor-verify`)、**`audit-security`**(只产出静态筛选报告)和 **`manage-assets`**(只出臃肿报告 —— 从不重写历史、从不删文件)。六个 skill —— `refactor-verify`、`setup-ci`、`write-for-ai`、`manage-secrets-env`、`project-conventions`、`unify-design` —— 直接点名调用时就是真正动手的 worker,被 sweep 拉进去时就切换成只读的汇报者。另外两个是仅 direct-call、永不参与并行 sweep 的:**`codex-fix`**(Claude Code 加 Codex 插件专用的薄壳,其他宿主下一行 fallback)和 **`ship-cycle`**(以 issue 驱动的编排器,需要 GitHub 加已认证的 `gh`,同样带优雅 fallback)。整个 pack 按认知可以分成两个类别:**代码卫生 worker 10 个**(sweep 覆盖的 9 个加上 `codex-fix`)和 **流程 worker 1 个**(`ship-cycle`)。
...
| [`ship-cycle`](#11-ship-cycle) | *"规划发布"*、*"把这些做成 issue 处理"*、*"用 issue 驱动开发"*、*"一起打包发一个版本"*、*"把 sweep 结果落成 issue"* | 以 issue 驱动开发的编排器 —— 起草多语言 issue、将其聚合为与 semver 版本一一对应的 milestone、根据 label 分派给对应的 worker 执行、从已关闭 issue 聚合 changelog 与 release notes、打带注释的 tag 并创建 GitHub release。**需要 Claude Code + GitHub + 已认证的 `gh` CLI**;其他场景下用一行优雅 fallback 退出 |
...
整个 pack 里第一个、而且故意是唯一一个特定宿主的 skill。它之所以存在,是因为一个特定维护者的一个特定工作流,发生得够频繁,足以占用一个 worker 槽位:*"我这一轮编辑结束了。用 Codex 跑一遍 second-model review。把 findings 返回来。让 Claude 带验证地解决它们。"* Codex 输出和 Claude 解决之间的复制粘贴一次一次地磨人,所以 `codex-fix` 把这一步自动化了。

它故意很薄 —— 大约 100 行 —— 因为所有实质的活都委托给了 `refactor-verify` 的 review-driven fix mode。这个 wrapper 自己只拥有 Codex 特有的粘合:host check、针对当前分支 diff 范围的 `/codex:rescue` 模板 prompt、输出收集、交接。findings 解析、real / false-positive / defer / duplicate 的分类、用 `git blame` 把每条 finding 对到一个 commit、依赖树计划、从叶子往上执行的 4-check 验证、带 review item back-reference 的提交 —— 这些全都是 `refactor-verify` 的事。

**宿主要求**。这个 skill 只在 **Claude Code 加 Codex 插件** 安装时触发。其他所有宿主下(Codex CLI 本身、Cursor、Copilot、Cline,或者没装 Codex 插件的 Claude Code),第一个动作就是一行 fallback 然后退出:*"未检测到 Codex 插件 —— 这个 skill 只在 Claude Code 加 Codex 下生效。要解决来自其他来源的 review,请把 findings 直接交给 `/refactor-verify`。"* 它绝不会挂住,也不会大声报错。

**`/codex-fix` 和直接调 `/refactor-verify` 怎么选**。如果你在 Claude Code 加 Codex 上,*而且* review 来源就是 Codex,用 `/codex-fix`。如果 review 来源是别的东西(人工 PR review、Sentry 告警、`gitleaks`、Semgrep、GitHub Advanced Security、手写的笔记),用 `/refactor-verify` 并把 findings 贴进去直接调。引擎是同一个,只是输入适配器不一样。

这是整个 pack 的第一个 **host-locked** skill,符合 [`docs/PHILOSOPHY.md`](./docs/PHILOSOPHY.md) 第 9 条 —— *"可移植的引擎可以有特定宿主的 wrapper"* —— 的模式。之后如果要为其他输入来源(Sentry、gitleaks 等)做 wrapper,应该照这个形状来:围绕可移植引擎的一层薄粘合、在 frontmatter 里显式声明宿主依赖、第一个动作是 graceful fallback。

### 11. `ship-cycle`

整个 pack 里第一个、目前也是唯一一个 **process 分类** 的 skill。其他 skill 作用于代码本身,这一个作用于代码外围的生命周期 —— issue、milestone、version、tag、release、changelog。它之所以存在,是因为以 issue 驱动开发是一条真实的工作流,手动做真的费时,而它沿途留下的那些数据点(结构齐整的 issue、带 back-reference 的 commit、跟 semver tag 对齐的 milestone)能让下一次 AI session 更快上手。

它跑的循环是:intake → draft → cluster → confirm → create → branch → execute → release。intake 接受三种输入形态 —— 粘贴进来的 findings、一份 `/vibesubin` sweep 报告,或一个具体范围。draft 按每个 item 填一份双语 issue 模板(Problem / Acceptance criteria / Implementation notes 带 skill 交接 / Linked)—— 正文语言一开始由你选(中文 / 英文 / 韩文 / 日文),章节标题保持英文以便 `gh` 解析稳定。cluster 套用 semver 决策树(bug / perf / 内部重构 / test / docs-only → patch;新增功能或新 skill → minor;破坏性 → major),每个 patch 上限约 5 条,保持 milestone 一致。confirm 给你看一张表(milestone、issue 数、体量估计)并等你点头,任何写操作都不会抢跑。create 每条跑一次 `gh issue create`,带 label 和 milestone;branch 按 `issue-<N>-<slug>` 命名;execute 根据每个 issue 的 `type` label 交给对应 worker —— bug 和 refactor 交给 `refactor-verify`,security 先过 `audit-security` 再接到 `refactor-verify`,docs 交给 `write-for-ai`,CI 交给 `setup-ci`,secrets 交给 `manage-secrets-env`,conventions 交给 `project-conventions`,design 交给 `unify-design`。release 把已关闭 issue 聚合成功能型 changelog,bump 两个 manifest,提交 `chore(release): vX.Y.Z`,打 annotated tag,push,把 release notes 写到临时文件,跑 `gh release create`,再用 `gh release view` 验证。

两件它不会做的事:跳过你对起草 issue 的审批(任何写操作都是 opt-in),以及在 main 上 CI 没绿或 `CHANGELOG.md` 还没更新之前就 push tag。它也不会为了图省事把不相关的 item 塞进同一个 milestone、不会在你选了中文的时候用英文模板,也不会通过"没经过交接 skill 验证"的 commit 去 auto-close issue。

**宿主要求**。只在 GitHub 仓库加已认证的 `gh` CLI 上跑(`gh auth status` 必须成功)。非 GitHub remote、`gh` 没登录,或者宿主里压根没装 `gh`,第一个动作就是一行 fallback 然后退出:*"`/ship-cycle` 需要 GitHub 仓库加已认证的 `gh` CLI —— 跑 `gh auth login`,或者在非 GitHub 宿主上直接调底层 worker(`/refactor-verify`、`/audit-security` 等)。"* 它绝不会挂住,也不会大声报错。

**`/ship-cycle` 和直接调底层 worker 怎么选**。当这次工作的单位是一个 **发布周期**(多条 item、需要跟踪、最后要 changelog 加 tag),用 `/ship-cycle`。当单位只是一次变更(一次重构、一个安全修复、一次文档重写),直接调对应 worker。`ship-cycle` 是编排层,它做的每件事,底层 worker 被点名时也都能自己做。
...
- **"编辑都结束了,用 Codex 跑一遍收尾"(只限 Claude Code 加 Codex 插件)**。`codex-fix` 在当前分支跑 `/codex:rescue` → 把 findings 交给 `refactor-verify` 的 review-driven fix mode → 分类、验证、带 back-reference 的 commit。在其他宿主,或者 review 来源不是 Codex(PR review、Sentry、扫描器、粘贴的笔记)的时候,直接把 findings 交给 `/refactor-verify` —— 引擎一样,只是输入路径不同。
- **规划一个发布(Claude Code + GitHub + `gh` 专属)。** 一批改动累积好了,或者 `/vibesubin` sweep 给出了优先修复列表 → `ship-cycle` 从列表起草多语言 issue → 聚合成对应下一个 semver 版本的 milestone → 按 label 把每个 issue 分派给对应 worker,带验证地执行 → milestone 关闭时,从已关闭 issue 聚合出功能型 changelog,bump 两个 manifest,打 annotated tag,创建 GitHub release。非 GitHub 宿主或 `gh` 未认证时 `ship-cycle` 以一行 fallback 退出 —— 此时直接调下层 worker。
