...
- **Don't recommend removing a vulnerable dependency without checking what depends on it.** Run `npm ls <pkg>` (or `pnpm why`, `yarn why`, `pip show`, `cargo tree -i`) to find dependents, and check whether a patched version exists before recommending removal. A package taken out without a replacement plan breaks downstream consumers. If a patched version exists, recommend the patch over the removal; if removal is the only path, surface the dependent list so the operator owns the breakage decision.
- **Don't recommend silencing the finding as a remediation.** Forbidden as a "fix": adding the file to `.semgrepignore` / `.banditignore` / `.trivyignore` without a CVE/justification comment, suppression comments (`# nosec`, `// trivy:ignore`, `# noqa: S` for security rules) without traceable rationale, downgrading severity to evade CI thresholds, removing the offending test instead of fixing the vulnerability. The audit measures real exposure; suppressions without traceable justification are a regression dressed as a fix.
