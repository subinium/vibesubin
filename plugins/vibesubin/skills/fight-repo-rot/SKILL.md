...
### Path exclusions — never flag generated code as dead

Apply these path-pattern exclusions before any dead-code analysis. Code under these paths is generated, vendored, or test-fixture, and should never be reported as a deletion candidate:

- `node_modules/`, `vendor/`, `target/`, `.venv/`, `dist/`, `build/`, `out/`, `.next/`, `__pycache__/`, `.tox/`
- `migrations/`, `db/migrate/`, `alembic/versions/` — schema migrations are intentionally never imported after they run; deleting them breaks rollback
- `tests/fixtures/`, `__fixtures__/`, `testdata/`, `*.fixture.*`, `*.golden.*`, `*.snap`, `__snapshots__/` — test fixtures are loaded by name from disk, not by static import
- Generated files matched by `// @generated`, `# AUTO-GENERATED`, `// THIS FILE IS AUTOGENERATED`, or comparable headers — regenerated on the next build, deleting them is a noop
- Story files (`*.stories.tsx`, `*.stories.ts`) — referenced by Storybook config, often not by import graph

When in doubt, `git log --follow <path>` — if the file's history is dominated by a single "regenerate" commit author or message, treat it as generated and exclude.
