...
Pack의 첫 번째이자 의도된 유일한 host 전용 skill. 한 사람의 한 가지 워크플로우가 충분히 자주 일어나서 worker 슬롯 하나를 쓸 만하다고 판단한 결과예요: *"편집 한 묶음 끝내고, Codex로 second-model 리뷰 돌리고, findings를 받아서 Claude가 검증과 함께 해결하게 한다."* Codex 출력과 Claude 해결 사이의 copy-paste가 session마다 마찰이 되어서, `codex-fix`가 그 단계를 자동화합니다.
...
**Host 요구사항.** 이 skill은 **Claude Code + Codex 플러그인 설치** 시에만 동작합니다. 다른 host (Codex CLI 자체, Cursor, Copilot, Cline, 혹은 Codex 플러그인 없는 Claude Code) 에서는 첫 액션이 한 줄 fallback과 exit: *"Codex 플러그인이 감지되지 않았습니다 — 이 skill은 Claude Code + Codex 전용입니다. 다른 소스의 리뷰로 작업하시려면 findings를 `/refactor-verify`에 직접 넘겨주세요."* 절대 hang되거나 소리내서 에러나지 않아요.
