...
- **직접 호출, 호스트 전용 (`/codex-fix`).** 하나의 워크플로우를 위한 얇은 wrapper: 현재 branch에 `/codex:rescue`를 돌려서 findings를 받고, `refactor-verify`의 review-driven fix mode에 넘겨서 4-check 검증과 함께 해결합니다. **Claude Code + Codex 플러그인 설치 시에만 동작**하고, 다른 host에서는 한 줄 fallback 남기고 깔끔히 빠져나옵니다. 포터블 엔진은 `/refactor-verify`에 있으니, Codex 말고 다른 리뷰 소스 (PR 리뷰, Sentry, 스캐너, 직접 붙여넣은 메모) 로 작업할 땐 `/refactor-verify`를 직접 부르세요.
- **직접 호출, 오케스트레이터 (`/ship-cycle`).** 팩의 첫 번째 process-카테고리 스킬입니다. 개선 인텐트 — 붙여넣은 노트든, `/vibesubin` sweep 리포트든, *"perf in src/api/"* 같은 범위 지정이든 — 를 받아서 잘 명세된 이중 언어 이슈 세트로 변환하고, semver 버전에 1:1로 매핑되는 마일스톤으로 묶고, 라벨별로 올바른 워커 스킬 (`refactor-verify`, `audit-security`, `write-for-ai` 등) 에 검증된 실행을 위임하고, 마일스톤이 닫히면 닫힌 이슈들을 기능 중심 changelog 항목으로 집계한 뒤 어노테이션 태그와 GitHub 릴리즈까지 컷합니다. 직접 호출 전용 — `/vibesubin` 병렬 sweep에는 들어가지 않습니다. **GitHub 리포 + 인증된 `gh` CLI 필요**; 다른 호스트 (GitHub 아닌 원격, 인증 안 된 `gh`, `gh` 미설치) 에서는 한 줄로 우아하게 폴백하고 종료합니다.

어떻게 부르든 절대 수정 안 하는 skill 세 개: **`fight-repo-rot`** (순수 진단 — dead code와 smell을 찾고, 삭제는 `refactor-verify`에 넘김), **`audit-security`** (static triage 리포트만), **`manage-assets`** (bloat 리포트만 — history 다시 쓰는 일도, 파일 지우는 일도 없음). 여섯 개 — `refactor-verify`, `setup-ci`, `write-for-ai`, `manage-secrets-env`, `project-conventions`, `unify-design` — 는 직접 호출하면 진짜 worker이고 sweep에서 불리면 read-only reporter입니다. 그리고 직접 호출 전용 skill 두 개는 sweep에 아예 참여하지 않습니다: **`codex-fix`** (Claude Code + Codex 플러그인 전용 얇은 wrapper — 다른 host에서는 한 줄로 우아하게 폴백), **`ship-cycle`** (이슈 드리븐 개발 오케스트레이터 — GitHub + 인증된 `gh` 필요, 마찬가지로 다른 host에서는 우아한 폴백). 팩은 인지적으로 구분된 두 카테고리를 지닙니다: **코드 위생 워커 10개** (sweep-eligible 9개 + `codex-fix`) 와 **프로세스 워커 1개** (`ship-cycle`).
...
| [`codex-fix`](#10-codex-fix) | *"codex 돌려서 고쳐줘"*, *"codex로 한번 검사하고 수정"*, *"codex driven fix"* | 얇은 post-edit wrapper — `/codex:rescue`를 현재 branch에 돌려서 findings를 받고, `refactor-verify`에 넘겨서 검증과 함께 해결. **Claude Code + Codex 플러그인 설치 시에만 동작**; 다른 host에서는 한 줄 fallback |
| [`ship-cycle`](#11-ship-cycle) | *"릴리즈 계획 짜자"*, *"이슈로 남기고 처리"*, *"이슈 드리븐으로 가자"*, *"이슈 묶어서 버전 올려"*, *"발견사항 이슈화"* | 이슈 드리븐 개발 오케스트레이터 — 다국어 이슈 초안 작성, 마일스톤=버전 매핑, 라벨별로 올바른 워커에 위임, 닫힌 이슈에서 체인지로그와 릴리즈 노트 집계, 어노테이션 태그와 GitHub 릴리즈까지. **Claude Code + GitHub + 인증된 `gh` CLI** 필요; 다른 호스트에선 한 줄로 우아하게 폴백 |
...
Pack의 첫 번째이자 의도적으로 유일한 host 전용 skill. 한 사람의 한 가지 워크플로우가 충분히 자주 일어나서 worker 슬롯 하나를 쓸 만하다고 판단한 결과예요: *"편집 한 묶음 끝내고, Codex로 second-model 리뷰 돌리고, findings를 받아서 Claude가 검증과 함께 해결하게 한다."* Codex 출력과 Claude 해결 사이의 copy-paste가 session마다 마찰이 되어서, `codex-fix`가 그 단계를 자동화합니다.

의도적으로 얇아요 — 100줄 정도 — 왜냐하면 실제로 substantial한 작업은 전부 `refactor-verify`의 review-driven fix mode에 위임하기 때문이에요. 이 wrapper는 Codex 전용 glue만 소유합니다: host check, 현재 branch diff로 스코프된 `/codex:rescue` 템플릿 프롬프트, 출력 수집, hand-off. findings 파싱, real / false-positive / defer / duplicate triage, `git blame`으로 각 finding을 commit에 매핑, dependency tree 계획, leaves-up 실행과 4-check 검증, review item으로 back-reference 커밋 — 전부 `refactor-verify`의 영역입니다.

**Host 요구사항.** 이 skill은 **Claude Code + Codex 플러그인 설치** 시에만 동작합니다. 다른 host (Codex CLI 자체, Cursor, Copilot, Cline, 혹은 Codex 플러그인 없는 Claude Code) 에서는 첫 액션이 한 줄 fallback과 exit: *"Codex 플러그인이 감지되지 않았습니다 — 이 skill은 Claude Code + Codex 전용입니다. 다른 소스의 리뷰로 작업하시려면 findings를 `/refactor-verify`에 직접 넘겨주세요."* 절대 hang되지 않고, 시끄럽게 에러를 뱉지도 않습니다.

**`/codex-fix`와 `/refactor-verify` 직접 호출 중 언제 뭘 쓰나.** Claude Code + Codex에 있고 리뷰 소스가 Codex이면 `/codex-fix`. 리뷰 소스가 다른 거면 (사람 PR 리뷰, Sentry alert, `gitleaks`, Semgrep, GitHub Advanced Security, 직접 쓴 메모) `/refactor-verify`를 findings 붙여넣어서 직접. 엔진은 같고, 입력 어댑터만 달라요.

이게 pack의 첫 **host-locked** skill이고, [`docs/PHILOSOPHY.md`](./docs/PHILOSOPHY.md) 규칙 9번 — *"포터블 엔진은 host 전용 wrapper를 가질 수 있다"* — 패턴을 따릅니다. 앞으로 다른 입력 소스 (Sentry, gitleaks 등) 를 위한 wrapper를 만들면 같은 모양이어야 합니다: 포터블 엔진 주변의 얇은 glue, host 의존성은 frontmatter에 명시, 첫 액션은 graceful fallback.

### 11. `ship-cycle`

팩의 첫 번째 process-카테고리 스킬. vibesubin의 다른 모든 스킬은 코드 자체를 대상으로 동작하는데, 이 스킬은 코드를 *둘러싼 라이프사이클* — 이슈, 마일스톤, 버전, 태그, 릴리즈, changelog — 을 다룹니다. 이슈 드리븐 개발은 손으로 하면 꽤 드는 비용이 있는 실제 워크플로우이고, 이 과정에서 남는 데이터 포인트들이 *다음* AI 세션을 더 빠르게 만들어주기 때문에 존재합니다.

루프는 이렇습니다: intake → draft → cluster → confirm → create → branch → execute → release.

**Intake**에서는 세 가지 경로를 받습니다 — 붙여넣은 개선 노트, `/vibesubin` sweep 리포트의 우선순위 리스트, 혹은 *"perf in src/api/"* 같은 이름 있는 범위. **Draft** 단계에서는 각 인텐트를 템플릿에 맞춰 이중 언어 이슈로 변환합니다: 한국어/영어 제목, 재현 또는 동기, acceptance criteria, 제안된 라벨 (`refactor`, `security`, `docs`, `ci`, `design`, `perf` 등), 추정 스코프, 후보 worker 스킬. **Cluster** 단계에서는 관련 이슈를 마일스톤 하나로 묶습니다 — semver 버전 하나에 1:1 매핑되고, 패치 릴리즈라면 이슈 ~5개가 상한입니다 (릴리즈가 너무 비대해지면 리뷰 불가능). **Confirm**은 승인 게이트입니다: 각 이슈 초안과 제안된 마일스톤/버전 바운더리를 사용자에게 보여주고, 승인 전에는 아무것도 생성하지 않습니다.

승인이 떨어지면 **Create**가 `gh issue create`로 마일스톤과 라벨을 붙여 이슈를 만들고, **Branch**는 이슈 번호를 포함한 명명 규칙 (`fix/123-foo`, `feat/124-bar`) 으로 브랜치를 깔고, **Execute**는 각 이슈를 라벨 → worker 매핑으로 올바른 스킬에 핸드오프합니다 (`refactor` → `refactor-verify`, `security` → `audit-security`, `docs` → `write-for-ai` 등). worker는 평소처럼 검증과 함께 일을 마치고 이슈를 닫습니다. **Release**는 마일스톤이 닫히면 닫힌 이슈들에서 기능 중심 changelog 항목을 집계하고, 두 매니페스트의 버전을 올리고, 어노테이션 태그를 컷하고, GitHub 릴리즈를 생성합니다.

안 하는 두 가지: 승인 게이트를 건너뛰지 않습니다 (초안을 그냥 생성해버리면 시끄러운 이슈 트래커가 됩니다). 그리고 CI 그린 + CHANGELOG 업데이트 확인 없이 태그를 푸시하지 않습니다 (버저닝이 결정적으로 유지되어야 다음 세션이 이 데이터를 신뢰할 수 있습니다).

**Host 요구사항.** 이 스킬은 **Claude Code + GitHub 리모트 + 인증된 `gh` CLI** 조합에서만 동작합니다. 비-GitHub 리모트, `gh` 미설치, 혹은 `gh auth status`가 실패하면 첫 액션이 한 줄 폴백과 종료입니다: *"이 스킬은 GitHub + 인증된 `gh` CLI가 필요합니다. 다른 환경에서는 해당 워커를 직접 호출하세요."* 조용히 실패하지 않고, 시끄럽게 에러를 뱉지도 않습니다.

**`/ship-cycle`과 직접 워커 호출 중 언제 뭘 쓰나.** 릴리즈 사이클 단위로 움직일 때 — *"이번 스프린트 개선사항 묶어서 다음 버전으로"*, *"`/vibesubin` 결과물로 이슈 만들고 처리"*, *"v1.2.0 계획 짜자"* — 는 `/ship-cycle`. 단일 변경 단위일 때 — *"이 파일 리팩토링"*, *"이 secret 점검"*, *"이 README 다시 써"* — 는 해당 워커 직접 호출. 엔진은 같고, 라이프사이클 오버헤드가 필요한지 여부만 다릅니다.
