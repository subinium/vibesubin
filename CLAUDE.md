...
2. **Never add a new worker skill past 10 — the cap is now reached.** The pack caps at 10 worker skills (the `vibesubin` umbrella is not counted in the 10). As of v0.3.2 all 10 slots are used: `refactor-verify`, `audit-security`, `fight-repo-rot`, `write-for-ai`, `setup-ci`, `manage-secrets-env`, `project-conventions`, `manage-assets`, `unify-design`, `codex-fix`. Any future new capability must extend an existing skill, split one, or displace one — do not expand the surface area. Users cannot remember more than ~10 tools.
